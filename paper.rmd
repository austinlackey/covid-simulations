...
In Ciufolini and Paolozzi's MonteCarlo Study, the authors stated that it is widely accepted that China's Covid-19 data follows a gauss-error function. To confirm that, we first used the China data from January 22, 2020 (included), to March 27, 2020 (included) to take a look. The figures below are the cumulative number of diagnosed positive cases of COVID-19 in China and the cumulative number of fatalities due to COVID-19 in China.
...
$$a+b*erf(c*x-d)$$ 
There are four parameters $a,b,c,d$ in this model, which shows great level of fit. Since they didn't specify how to realize the function in R, we went around and found that the $nls$ function did a great job on what we need. We took a cheap gauss of $a,b,c,d$ from the preview of the data as the start points, and apply the $nls$ function to determine the nonlinear (weighted) least-squares estimates of the four parameters $a,b,c,d$. Here is an example code of the model construction by the $nls$ function:
...
In this section, we reported the results of the fit of China's data. The figures below are the fit of the cumulative number of diagnosed positive cases of COVID-19 in China and the fit of the cumulative number of fatalities due to COVID-19 in China. The red solid lines are the fitting functions of the type of a Guass error function with four parameters. The horizontal axis shows the day numbers from January 22, 2020 (included), to March 27, 2020 (included). The vertical axis shows the cumulative numbers.
